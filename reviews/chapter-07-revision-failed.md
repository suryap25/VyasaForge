...
**Token-Based Authentication: JWT vs. Opaque Tokens**

When implementing token-based authentication, consider the trade-offs between JWT and opaque tokens:

**JWT (JSON Web Tokens)** are self-contained and stateless. The server signs the token, and the client includes it with requests. The server validates the signature without database lookups. However, JWT has security implications: the token payload is base64-encoded (not encrypted), so sensitive data in claims is visible to anyone with the token. Additionally, JWT is vulnerable to algorithm confusion attacks if not carefully validated. JWTs cannot be revoked immediately—they remain valid until expiration.

**Opaque Tokens** are random strings that reference server-side session data. The server stores the token and associated user information, then validates tokens by looking them up in storage. Opaque tokens can be revoked immediately and don't expose claims to the client. However, they require server-side storage and database lookups on every request, which impacts scalability.

**Recommendation:** For high-security applications (financial services, healthcare), use opaque tokens with server-side validation. For distributed systems requiring stateless authentication, use JWT with short expiration times and implement refresh token rotation. Never store sensitive data in JWT claims. Always validate JWT signatures using the correct algorithm and key.
...
def verify_mfa_code(secret: str, code: str) -> bool:
    ...
    totp = pyotp.TOTP(secret)
    # valid_window=1 allows ±30 seconds of clock skew between client and server,
    # which is essential because user devices may have slightly inaccurate time
...
Magic links provide a passwordless authentication mechanism suitable for web applications:
...
def request_magic_link(email: str) -> bool:
    ...
    # Generate a secure, short-lived magic link token
    ...
    # Store magic token (use a separate store, not session storage)
    magic_store.set(magic_token, magic_data, ex=900)  # 15 minutes
    
    # Send magic link via email
    magic_link = f"https://yourapp.com/auth/magic?token={magic_token}"
    send_email(user.email, "Your Login Link", magic_link)
    
    return True

def authenticate_with_magic_link(magic_token: str) -> Optional[User]:
    """Authenticate a user using a magic link token."""
    magic_data = magic_store.get(magic_token)
    
    if not magic_data:
        return None
    
    # Check expiration
    if datetime.utcnow() > magic_data['expires_at']:
        magic_store.delete(magic_token)
        return None
    ...
    # Mark token as used
    magic_data['used'] = True
    magic_store.set(magic_token, magic_data, ex=900)
    
    # Authenticate the user
    user = database.get_user(magic_data['user_id'])
    return user
```

Magic links must be transmitted only over HTTPS in URLs. The token should be cryptographically random, single-use, and short-lived (10-15 minutes). After successful authentication, create a session or token as with traditional authentication.
...
def request_password_reset(email: str) -> bool:
    ...
    reset_store.set(reset_token, reset_data,## Pentest Lens
