...
- **Session Fixation**: Determine if an attacker can force a user to use a known session token. Obtain a session token before authentication (from login page or error message), trick the user into authenticating with that token (via URL parameter or cookie injection), then use the same token to impersonate the user. Secure applications regenerate session tokens after successful authentication, invalidating the attacker's pre-authentication token.

- **Session Prediction**: Analyze session tokens for patterns. Are they sequential? Timestamp-based? Predictable? Capture multiple valid session tokens and analyze for mathematical patterns or weak randomization.

- **Session Hijacking**: Attempt to use captured or guessed session tokens. Can an attacker use a valid session token to impersonate a user? Test by capturing tokens from network traffic (if not HTTPS), browser storage, or logs.

- **Session Timeout**: Verify that sessions expire appropriately. Can an attacker use an old session token after logout or timeout? Test by capturing a token, waiting for the session to expire, then attempting to use the expired token.

- **Secure Cookie Attributes**: Verify that session cookies have appropriate security attributes. Test for HttpOnly (prevents JavaScript access), Secure (HTTPS-only transmission), and SameSite (CSRF protection). Attempt to access session cookies via JavaScript; if successful, HttpOnly is missing.

**Multi-Factor Authentication Testing**: If MFA is implemented, test for bypasses:

- Can MFA be disabled or skipped through alternative authentication methods?
- Can MFA codes be reused or brute-forced?
- Are MFA codes transmitted securely and not logged in plaintext?
- Can MFA be bypassed by modifying requests (removing MFA parameters, changing authentication flow)?
- Are backup codes stored securely and rate-limited?
- Can backup codes be enumerated or guessed?
...
2. Identify resource identifiers (user IDs, document IDs, UUIDs, etc.)
...
5. Test across different endpoints and HTTP methods (GET, POST, PUT, DELETE)
6. Document any successful unauthorized access
```
...
4. Attempt to modify parameters to escalate privileges (role=admin, is_admin=true)
5. Test for missing authorization checks on sensitive operations
6. Attempt to invoke admin functions through indirect means (API calls, parameter manipulation)
```
...
4. Test with sequential IDs, UUIDs, GUIDs, and other identifier formats
5. Test across different endpoints and HTTP methods
6. Test with indirect references (document IDs that map to user data)
```

**Authorization Policy Testing**: Examine authorization policies for logical flaws:

- Can permissions be combined to achieve unintended access? (e.g., read + write = delete)
- Are there race conditions in permission checks? (check-then-act vulnerabilities)
- Can permissions be bypassed through parameter manipulation or request modification?
- Are there inconsistencies between different authorization checks across endpoints?
- Can authorization be bypassed by modifying HTTP methods (GET vs. POST)?
- Are there timing-based bypasses where authorization checks can be skipped during race conditions?"
    },
    {## Common Findings
...
**Finding 2: Weak or Predictable Session Tokens**

**Description**: Session tokens are generated using weak randomization, sequential patterns, or predictable algorithms, allowing attackers to guess valid session tokens.

**Example**: A web application generates session tokens using `session_id = user_id + timestamp`. An attacker can predict future session tokens by knowing a user's ID and the current time.

**Impact**: Session hijacking and account takeover. An attacker can forge valid session tokens and impersonate any user.
...
def store_session_token(user_id, token):
    ...
    Session.create(user_id=user_id, token_hash=token_hash)
    return token
...
**Finding 3: Missing CSRF Protection on State-Changing Operations**

**Description**: State-changing operations (POST, PUT, DELETE) do not validate CSRF tokens, allowing attackers to trick users into performing unintended actions on other websites.

**Example**: A banking application's transfer endpoint does not validate CSRF tokens. An attacker creates a malicious website with `<img src="https://bank.com/transfer?to=attacker&amount=1000">`. When a logged-in user visits the attacker's site, the transfer executes without the user's knowledge.

**Impact**: Unauthorized state changes (fund transfers, password changes, permission modifications) performed on behalf of authenticated users.
...
def transfer_funds():
    ...
    process_transfer(user.id, recipient, amount)
    return "Transfer successful"
...
Token revocation is critical for logout, permission changes, and incident response. Two primary patterns exist:

**Blacklist Pattern** (suitable for short-lived
