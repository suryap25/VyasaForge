...
**Storing MFA State in Client-Side Tokens**: Applications that encode the MFA challenge state into JWT tokens or cookies without server-side validation allow attackers to forge authentication states. MFA challenge state must be maintained server-side.

**Insufficient Challenge Entropy**: MFA codes generated with insufficient randomness (e.g., sequential codes, predictable patterns) can be brute-forced. Use cryptographically secure random number generators.

**Missing Constant-Time Comparison**: Comparing MFA codes using standard string comparison functions can leak timing information about correct digits. Always use constant-time comparison functions.

**Inadequate Rate Limiting Scope**: Rate limiting that only applies per-challenge but not per-user allows attackers to generate multiple challenges and attempt codes across all of them. Implement both per-challenge and per-user rate limiting.

**Weak Recovery Mechanisms**: Account recovery processes that don't require MFA verification (e.g., security questions only) become the weakest link in the authentication chain. Recovery should require equivalent security to MFA itself."
    },
    {## Developer Lens
...
- Use consistent session attributes to track MFA verification across## Secure Design Guidance
