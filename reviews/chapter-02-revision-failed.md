...
**SAML Vulnerabilities** in federated systems include XML signature wrapping attacks where attackers modify SAML assertions by wrapping legitimate signatures around malicious content, XXE (XML External Entity) injection in SAML parsers, and signature validation bypasses. For example, an attacker might craft a SAML response that claims to be from a trusted identity provider but contains forged user attributes.

**OpenID Connect Issues** include missing or improper nonce validation (allowing replay attacks), insufficient ID token validation (accepting tokens with invalid signatures or expired timestamps), and state parameter bypass in authorization flows. Applications must validate that the nonce in the ID token matches the nonce sent in the authorization request to prevent token replay.

**Trust Relationship Exploitation** occurs when attackers compromise or impersonate identity providers, or when service providers incorrectly validate identity provider signatures. If an application trusts multiple identity providers but doesn't properly verify which provider issued a token, attackers might forge tokens from untrusted providers.

**Cross-Domain Identity Spoofing** happens when attackers exploit weak validation of identity provider metadata or certificates. For example, if an application accepts identity assertions from any provider with a valid signature (rather than a whitelist of trusted providers), attackers can create their own identity provider and issue themselves valid assertions.

**Redirect and Phishing in Federation** includes open redirect vulnerabilities in OAuth/OIDC flows where the redirect_uri parameter is not validated, allowing attackers to redirect users to phishing sites after authentication. Additionally, attackers might register lookalike identity providers to intercept authentication flows."
    },
    {## Developer Lens
...
Secure IAM design requires systematic thinking about access control models, authentication mechanisms, and authorization enforcement patterns.

### Selecting an Access Control Model

Choose between RBAC and ABAC based on your authorization complexity:

**Role-Based Access Control (RBAC)** is appropriate when:
- Authorization decisions depend primarily on user roles
- The number of distinct roles is manageable (typically under 50)
- Permissions are relatively stable and don't change frequently
- Fine-grained resource-level decisions are not needed

Example: A content management system with Editor, Reviewer, and Publisher roles.

**Attribute-Based Access Control (ABAC)** is appropriate when:
- Authorization depends on multiple attributes (user department, resource classification, time of day, location)
- Permissions are dynamic or context-dependent
- Fine-grained resource-level control is required
- You need to express complex policies (e.g., "Allow access if user is in healthcare department AND resource is marked internal AND current time is business hours")

Example: A healthcare system where access to medical records depends on user role, department, patient consent, and audit requirements.

### Centralized vs. Distributed Authorization

**Centralized Authorization** stores all authorization policies in a single service or policy engine:
- Advantages: Consistent policy enforcement, easier auditing, simpler policy updates
- Disadvantages: Single point of failure, potential performance bottleneck, network dependency
- Best for: Monolithic applications, applications with moderate authorization complexity

**Distributed Authorization** embeds authorization logic in individual services:
- Advantages: Resilient to central service failures, better performance, services can evolve independently
- Disadvantages: Policy inconsistency, harder to audit, policy duplication
- Best for: Microservices architectures, high-availability requirements

Hybrid approach: Use a centralized policy engine for policy definition and distribution, but cache policies locally in each service for performance and resilience.

### Session vs. Token Trade-offs

| Aspect | Server-Side Sessions | Stateless Tokens (JWT) |
|--------|----------------------|------------------------|
| **Scalability** | Requires session store; harder to scale horizontally | Stateless; scales easily |
| **Revocation** | Immediate; delete session from store | Difficult; requires revocation list or waiting for expiration |
| **Bandwidth** | Session ID only (small) | Full token with claims (larger) |
| **Offline Use** | Not possible | Possible if token is self-contained |
| **Cross-Domain** | Requires shared session store or federation | Works across domains naturally |
| **Security** | Server controls all state | Client stores token; risk of tampering |

**Decision Matrix**:
- Use server-side sessions for: Web applications, applications requiring immediate revocation, applications with centralized infrastructure
- Use tokens for: APIs, distributed systems, mobile applications, cross-domain scenarios
- Use hybrid (short-lived access token + refresh token) for: High-security applications needing both scalability and revocation

### Federated Identity Considerations

When designing systems that use external identity providers:

1. **Validate Everything**: Verify identity provider signatures, validate token expiration, check nonce values, and validate redirect URIs against a whitelist.

2. **Establish Trust Carefully**: Maintain a whitelist of trusted identity providers. Don't accept assertions from any provider with a valid signature.

3. **Map External Identities Securely**: When linking external identities to local accounts, use secure identifiers (not email addresses which can change). Implement account linking workflows that verify user consent.

4. **Handle Provider Failures**: Design for identity provider unavailability. Consider fallback authentication mechanisms or cached identity information.

5. **Audit Federation Events**: Log all federation-related events (successful authentications, failed attempts, account linking) for security monitoring.

### IAM Threat Modeling

When designing IAM systems, consider these threat scenarios:

1. **Credential Compromise**: Assume credentials will be compromised. Design for detection (unusual login patterns, impossible travel) and response (forced re-authentication, session termination).

2. **Session Theft**: Assume session identifiers or tokens might be stolen. Use short timeouts, secure transmission (HTTPS), and secure storage (HttpOnly cookies).

3. **Authorization Bypass**: Assume attackers will attempt to access resources they shouldn't. Implement consistent authorization checks at the resource level, not just the endpoint level.

4. **Privilege Escalation**: Assume attackers will attempt to gain higher privileges. Never trust client-provided role or permission information; always verify against server-side data.

5. **Identity Spoofing**: Assume attackers will attempt to impersonate other users or services. Implement strong authentication and validate all identity assertions.

### Design Review Checklist

When reviewing IAM system designs, verify:

**Authentication**
- [ ] Credentials are never stored in plaintext
- [ ] Password hashing uses bcrypt, scrypt, or Argon2 with appropriate cost factors
- [ ] MFA is available for sensitive operations or high-value accounts
- [ ] Failed authentication attempts are logged and monitored
- [ ] Credential transmission is encrypted (HTTPS)
...
- [ ] Authorization logic is centralized or consistently implemented
- [ ] Least privilege principle is applied (users have minimum necessary permissions)
...
- [ ] Sessions have appropriate timeout periods (both absolute and idle)
- [ ] Session cookies use Secure, HttpOnly, and SameSite flags
- [ ] Sessions are invalidated on logout

**Token-Based Authentication**
- [ ] Tokens include expiration claims and are validated
- [ ] Token signatures are verified before trusting token contents
- [ ] Sensitive data is not stored in token payloads
- [ ] Token revocation mechanisms are implemented
- [ ] Tokens are transmitted securely (HTTPS, Authorization header)

**Federated Identity**
- [ ] Identity provider signatures are validated
- [ ] Redirect URIs are validated against a whitelist
- [ ] State parameters are used and validated for CSRF protection
- [ ] Nonce values are used and validated for replay protection
- [ ] External identities are securely mapped to local accounts"
    },
    {## Interview Questions
...
- You discover an API endpoint checking `if request.headers.get('X-User-Role') == 'admin'` to enforce authorization. What is the vulnerability and how would you fix it?

- Walk me through how you would test a multi-tenant SaaS application for horizontal privilege escalation vulnerabilities. What specific test cases would you run?

- A developer argues that storing user roles in a JWT is acceptable because "the signature prevents tampering." What is wrong with this reasoning and what is the correct approach?

- How would you design session management for a high-traffic API that must scale horizontally across 50 servers? What are the trade-offs between server-side sessions and tokens in this scenario?

- Describe the difference between token revocation in server-side sessions versus JWTs. What are the security and operational trade-offs of each approach?

- You find that an application stores user permissions in a client-side cookie
