...
**Design Principle: Defense in Depth for Mobile Authentication**

Mobile authentication security requires layered controls across device, network, and backend systems. No single mechanism—not even certificate pinning or biometric authentication—provides complete protection. Design systems assuming device compromise is possible.

**Token Lifecycle Design**

Structure token lifecycles to minimize exposure:

- **Access tokens** should expire within 15 minutes to 1 hour. Short lifespans limit the window an attacker has to use a stolen token.
- **Refresh tokens** should expire within days to weeks and must be rotated on each use. Implement refresh token rotation: when a refresh token is used to obtain a new access token, issue a new refresh token and invalidate the old one.
- **ID tokens** (OpenID Connect) should never be used for API authorization. Use them only to obtain user identity claims after authentication.
- Implement token revocation endpoints that immediately invalidate tokens server-side, independent of client-side deletion.

**Secure Storage Design**

Never assume the device is trustworthy:

- Store tokens only in platform-provided secure storage: Keychain (iOS) with `kSecAttrAccessibleWhenUnlockedThisDeviceOnly` or Keystore (Android) with `EncryptedSharedPreferences`.
- Never store credentials (passwords, API keys, client secrets) on the device after authentication. Use credentials only during the authentication request, then discard them.
- Implement secure deletion: when logging out, explicitly clear tokens from secure storage using platform APIs (SecItemDelete on iOS, SharedPreferences.edit().remove() on Android).
- Design for device loss: assume tokens may be extracted from a lost device. Implement server-side session tracking and device binding to detect and revoke compromised tokens.

**Network Security Design**

Mobile devices connect through untrusted networks. Design accordingly:

- Enforce TLS 1.2 or higher for all authentication traffic. Reject connections using older TLS versions.
- Implement certificate pinning for authentication endpoints. Pin both the leaf certificate and intermediate certificates to handle certificate rotation.
- Validate certificate chains completely: verify the certificate is signed by a trusted CA, has not expired, and matches the expected hostname.
- Consider implementing public key pinning as a fallback if certificate pinning becomes difficult to maintain.

**Device Binding Design**

Bind tokens to specific devices to prevent token reuse across devices:

- Include a device identifier in token claims (JWT `device_id` claim or opaque token metadata).
- Generate device identifiers using platform-provided mechanisms: `UIDevice.current.identifierForVendor` on iOS or `Build.SERIAL` on Android. Do not use IMEI or other hardware identifiers that persist across factory resets.
- Validate device binding server-side: reject tokens presented from devices with mismatched identifiers.
- Implement device registration: require users to authorize new devices before they can authenticate, allowing users to see which devices have access to their accounts.

**Multi-Factor Authentication Design**

Biometric authentication enhances user experience but does not replace strong authentication:

- Biometric authentication authenticates the user to the device, not to the application. After biometric verification, establish a proper authenticated session using tokens.
- Do not use biometric authentication as the sole authentication mechanism. Require biometric authentication plus token-based authentication.
- Implement biometric authentication using platform-provided APIs: LocalAuthentication framework on iOS, BiometricPrompt on Android. These APIs handle biometric sensor interaction securely.
- Allow users to disable biometric authentication and fall back to password-based authentication.
- Implement timeout: require re-authentication (biometric or password) if the application has been inactive for a defined period (typically 5-15 minutes).

**OAuth 2.0 PKCE Design**

For applications using OAuth 2.0:

- Always use the Authorization Code flow with PKCE. Never use the Implicit flow or Resource Owner Password Credentials flow on mobile.
- Generate code_verifier as a cryptographically random string of 43-128 characters using platform-provided random number generators.
- Generate code_challenge as the SHA256 hash of code_verifier, base64url-encoded.
- Include state parameter in authorization requests and validate it in callbacks to prevent CSRF attacks.
- Validate redirect URIs strictly: use custom URL schemes (e.g., `com.example.app://oauth-callback`) or universal links (iOS) / app links (Android) to prevent authorization code interception by malicious applications.

**Logout and Session Revocation Design**

Logout must be comprehensive:

- Clear all tokens from secure storage when the user logs out.
- Notify the backend to revoke the user's session and invalidate all tokens.
- Implement server-side session tracking: maintain a list of active sessions per user and allow users to revoke sessions from other devices.
- Implement automatic session revocation: revoke sessions if suspicious activity is detected (e.g., login from a new device, geographic impossibility).
- Design for offline scenarios: if the application supports offline functionality, implement local session state that is synchronized with the server when connectivity is restored.

**Testing and Validation Design**

Build security testing into the development lifecycle:

- Implement automated tests that verify tokens are stored in secure storage, not in accessible files.
- Test token expiration and refresh token rotation in integration tests.
- Perform manual penetration testing on actual devices, not just emulators.
- Test certificate pinning bypass scenarios to ensure pinning cannot be easily circumvented.
- Implement logging and monitoring: log authentication events (login, logout, token refresh, failed authentication) server-side for security analysis, but never log tokens or credentials.## Interview Questions
...
- Mobile authentication differs fundamentally from web authentication due to device compromise risks, untrusted networks, and the ability to extract tokens from secure storage on rooted/jailbroken devices.
- Token-based authentication (OAuth 2.0 with PKCE, JWT) is the standard for mobile applications because tokens work across network transitions, support offline scenarios, and reduce server-side session state.
- Secure token storage is critical: use Keychain (iOS) and Keystore (Android), never SharedPreferences or UserDefaults. Tokens in accessible storage can be extracted by attackers on compromised devices.
- Credentials (passwords, API keys) should never be stored on the device after authentication. Use credentials only during the authentication request, then discard them immediately.
- Certificate pinning prevents man-in-the-middle attacks on untrusted networks by validating that the server's certificate matches an expected certificate or public key, not just any certificate signed by a trusted CA.
- Device binding—including device identifiers in token claims and validating them server-side—prevents attackers from using tokens extracted from one device on another device.
- Biometric authentication enhances user experience but authenticates the user to the device, not to the application. Establish a proper authenticated session with tokens after biometric verification.
- OAuth 2.0 PKCE (Proof Key for Public Clients) prevents authorization code interception attacks specific to mobile by requiring a code_verifier that only the legitimate application can generate.
- Token lifecycle design matters: short-lived access tokens (15 minutes to 1 hour) limit exposure if compromised, while refresh tokens should be rotated on each use to prevent indefinite access after compromise.
- Logout must be comprehensive: clear tokens from secure storage, revoke sessions server-side, and implement device-level session management so users can revoke access from lost or stolen devices.
- Mobile authentication security requires defense in depth: no single mechanism (not even certificate pinning or biometric authentication) provides complete protection. Layer controls across device, network, and backend systems.## Sketchnote Placeholder

[SKETCHNOTE DIAGRAM PLACEHOLDER]

## Interview Questions

- How would you verify that tokens are properly bound to a specific device and cannot be reused on another device?
- What is the security difference between storing an access token versus a refresh token on a mobile device, and why should they be handled differently?
- Describe how you would test whether certificate pinning is actually preventing man-in-the-middle attacks, not just being bypassed by the testing proxy.
- What is the difference between biometric authentication (authenticating the user to the device) and biometric authorization (using biometrics to authorize API requests), and why does this distinction matter?
- If you discovered that a mobile application stores credentials in SharedPreferences or UserDefaults after authentication, what would be your remediation approach and why is this a critical finding?
- How would you design a token refresh mechanism that prevents refresh token reuse attacks while maintaining a good user experience?
- What are the security implications of implementing OAuth 2.0 without PKCE on mobile, and how would you test for this vulnerability?
- How would you test whether the state parameter in an OAuth 2.0 flow is properly validated, and what attack would you be preventing?
- Describe a scenario where a user's device is rooted or jailbroken. What authentication controls would still protect the user's account, and which would be compromised?
- How would you implement device registration and revocation so that users can see which devices have access to their accounts and revoke access to lost or stolen devices?
