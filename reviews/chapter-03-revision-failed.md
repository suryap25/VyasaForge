...
When access tokens expire, use the refresh token to obtain a new access token without requiring the user to re-authenticate.
...
def refresh_access_token():
    ...
    # Issue new tokens in HTTP-only cookies
    resp = jsonify({'success': True})
    resp.set_cookie(
        'access_token',
        token_data['access_token'],
    ...
    # Rotate refresh token if new one issued
    if 'refresh_token' in token_data:
        resp.set_cookie(
            'refresh_token',
            token_data['refresh_token'],
...
When users log out, revoke tokens to prevent further use.
...
def logout():
    ...
    if refresh_token:
        try:
            requests.post(
                'https://auth-server.example.com/revoke',
                data={'token': refresh_token},
                timeout=5
            )
        except requests.RequestException:
            pass  # Log but don't fail logout
...
- **Web applications**: Store tokens in memory or HTTP-only, Secure cookies. HTTP-only cookies prevent JavaScript access, protecting against XSS token theft. Secure flag ensures transmission only over HTTPS. SameSite attribute provides CSRF protection.
- **Mobile applications**: Use platform-specific secure storage (Keychain on iOS, Keystore on Android). Never store tokens in shared preferences or app-accessible directories.
- **Never use browser local storage**: LocalStorage is accessible to any JavaScript on the page, including malicious scripts injected via XSS.

**3. Validate All Inputs**: Validate authorization codes, state parameters, and redirect URIs on the backend before processing. Never trust client-side validation.

**4. Handle Token Expiration Gracefully**: Implement automatic token refresh before expiration. When refresh fails, redirect users to re-authenticate rather than failing silently.

**5. Implement Error Handling**: Handle authorization server errors, network timeouts, and## Pentest Lens
